...
```{r}
library(tidyverse)
non_smoker <- bwt |> filter(smoker=='No') |>  pull(Birthweight)
mean(non_smoker)
t.test(non_smoker, conf.level = 0.95)
```

Para SI fumadores

```{r}
smoker <- bwt |> filter(smoker=='Yes') |>  pull(Birthweight)
mean(smoker)
t.test(smoker, conf.level = 0.95)
```

```{r}
bwt |> ggplot(aes(x=Birthweight, color=smoker)) + geom_density()
```
...
probs <- 1 - (dbinom(0, size=x, p=p)+dbinom(1, size=x, p=p))
plot(x,probs)
```
...
```{r}
model1 <- lm(formula=aparicion~gen_total, data=dflog)
model2 <- lm(formula=aparicion~gen_reduc, data=dflog)
```
...
```{r}
model3 <- lm(formula=gen_reduc~aparicion, data=dflog)
summary(model3)
```