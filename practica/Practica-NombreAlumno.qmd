...
```{r}
# 8.Eliminar todos los barrios con todas las rooms con NA Square meters (4901 apartamentos)
...
plot(color_branches(sqm.dend, h=0.25),main = "Dendrograma de Barrios por Metros Cuadrados", leaflab = "none")

text(1:length(labels(sqm.dend)), 
     par("usr")[3] - 0.02, 
     labels = labels(sqm.dend), 
     srt = 90, 
     adj = c(1, 0.5), 
     xpd = TRUE, 
     cex = 1.2)
...
```{r}
df_clusters <- data.frame(Neighbourhood = names(clusters), neighb_id = as.numeric(clusters))
str(df_clusters)
...
```{r}
#13. Predecir Sqm en función del resto de variables. Hacemos la matriz de correlacion quitando los barrios y los pies cuadrados. 
