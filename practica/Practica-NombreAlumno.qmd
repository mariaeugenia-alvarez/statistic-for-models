...
```{r}
# 9. El p-valor del test shapiro (2.2e-16) es muy bajo, podemos descartar la hipotesis nula, es decir los datos NO siguen una distribución normal
...
df_madrid.train<-df_madrid_cluster[idx,]
df_madrid.test <-df_madrid_cluster[-idx,]
...
library(GGally)
ggpairs(df_madrid[, -c(1, 7)], 
      lower = list(continuous = wrap("points", alpha = 0.3,size=0.1,color='blue'))
       )
# Según esta matriz las variables como Latitude, Longitude y Review.Scores.Rat tienen una correlación muy baja con Square.Meters por lo que no las incluimos en el modelo.
```

```{r}
#13. Predecir Sqm en función del resto de variables. 
model_sqm<-lm(Square.Meters~Accommodates+Bathrooms+Bedrooms+Beds+Price+neighb_id,data=na.omit(df_madrid.train))
summary(model_sqm)

#vemos que de todas las variables, las que tienen un p-valor muy significativo son bathrooms y price y significativo aunque algo menos bedrooms. El barrio 3 tiene apartamentos 82m2 más grandes que el barrio 1
#el modelo explica el 75% de la variabilidad (Rsquared=0.75) con un error de predicción de +-26.86m2
...
paste("RMSE:", sqrt(mean((df_madrid.train$Square.Meters-df_madrid.train$sqm_est)^2)))
