...
```{r}
# 7.NA <20 Square meters (45 apartamentos por debajo de 20m2 son en total 5427)
# según vemos en el histograma, hay muy pocos apartamentos mayores de 200 m2 que añaden más ruido que valor, así que los eliminamos junto a los de menos de 20m2 quedando un total de 4864
df_madrid <- df_madrid %>% mutate(Square.Meters = ifelse(Square.Meters < 20 | Square.Meters > 200, NA, Square.Meters))
```

```{r}
# 8.Eliminar todos los barrios con todas las rooms con NA Square meters (4864 apartamentos)
...
```{r}
# 9. El p-valor del test shapiro (1.4e-08) es muy bajo, podemos descartar la hipotesis nula, es decir los datos NO siguen una distribución normal
...
```{r}
# 11. Precedir dendograma a partir de la matriz de Tukey. Cortamos en 0.25 y aparecen 3 clusters
...
# Según esta matriz las variables como Latitude, Longitude y Review.Scores.Rat tienen una correlación muy baja con Square.Meters por lo que no las incluimos en el modelo. Extra-people y guests también las descartamos por ser también menores de 0.5
```
...
#vemos que de todas las variables, las que tienen un p-valor muy significativo son bedrooms, accommodates y los barrios 2 y 3
#bedrooms y accommodates estan muy relacionadas por lo que nos quedamos con la primera 
#el modelo explica el 70% de la variabilidad (Rsquared=0.7) con un error de predicción de +-15.73m2
...
#vemos que siguen una distribución normal
...
#nos da un valor de R2 para el testing de 0.64 y además el RMSE es mayor que en training (20.25m2) lo que puede indicar overfitting. 
...
model_sqm2<-lm(Square.Meters~Bedrooms+neighb_id,data=df_madrid.train)
summary(model_sqm2)

#nos da un valor de R2 para testing de 0.67, por lo que hemos mejorado y además el RMSE ha mejorado algo (19.3m2). El error estandar también ha bajado ligeramente
...
sqm_predict <- predict(model_sqm2, newdata = data.frame(Bedrooms = 3, neighb_id = df_clusters[df_clusters$Neighbourhood == "Sol", "neighb_id"]))
