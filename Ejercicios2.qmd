...
```{r}
m <- mean(bwt$Birthweight[bwt$smoker == "NO"])
s <- sd(bwt$Birthweight[bwt$smoker == "NO"])
p <- 0.05
sm <- s/sqrt(length(bwt$Birthweight[bwt$smoker == "NO"]))
...
```{r}
t.test(bwt$Birthweight[bwt$smoker == "NO"])
```

Para SI fumadores

```{r}
mean(bwt$Birthweight[bwt$smoker == "YES"])
```

```{r}
t.test(bwt$Birthweight[bwt$smoker == "YES"])
```
